...
from core.refresh_task_store import refresh_task_store
router = APIRouter(prefix=f"/articles", tags=["文章管理"])


def _set_refresh_task(task_id: str, data: dict):
    return refresh_task_store.set_task(task_id, data)


def _get_active_refresh_task(article_id: str):
    return refresh_task_store.get_active_task(article_id)
    ...
    try:
        ...
        if not updated:
            _set_refresh_task(task_id, {
                ...
                "article_id": article_id,
                "status": "failed",
                "message": f"文章刷新失败: mode={fetch_mode}"
            })
            return
...
async def get_refresh_task_status(
    ...
    task = refresh_task_store.get_task(task_id)
    if not task:
        raise HTTPException(
            status_code=fast_status.HTTP_404_NOT_FOUND,
            detail=error_response(
